...
public class MediaPlayerTests
{
    ...
    /// <summary>
    /// ⚠ <see cref="MediaAccessOptions.AllowedRoots"/> is the containment boundary, so its DEFAULT must be
    ...
    public void Conversion_is_off_until_roots_are_named()
    {
        ...
        Assert.Empty(options.Access.AllowedRoots);
        Assert.Null(options.ResolveUri);
        // Empty, not null: `Access.CacheRoot` is a non-nullable placeholder that `UseMediaPlayer` reads as
        // "not chosen yet" and replaces with `paths.DataArea("media")` — see `MediaPlayerOptions.Access`.
        Assert.Empty(options.Access.CacheRoot);
    }
