...
public class CapabilityOverrideTests
{
    ...
    public void A_player_substituted_in_the_callback_is_the_one_that_resolves()
    {
        ...
        builder.UseMediaPlayer((options, services) =>
        {
            ...
            {
                Resolve = static _ => null, AllowedRoots = [@"C:\MyApp\media"], CacheRoot = "",
            };
    ...
    public void Options_set_beside_a_substitution_still_reach_the_container()
    {
        ...
        builder.UseMediaPlayer((options, services) =>
        {
            // ⚠ `Access` REPLACED WHOLESALE rather than a single field mutated on purpose:
            // `MediaAccessOptions.CacheRoot` is `{ get; init; }`, like every member on that shared type, so
            // `options.Access.CacheRoot = …` is CS8852 — the same trap the kit already documented for its
            // other options records. `MediaPlayerOptions.Access` itself stays `{ get; set; }` precisely so
            // this callback (and `UseMediaPlayer`'s own deferred-default step) CAN swap the whole object in.
            options.Access = new MediaAccessOptions { Resolve = static _ => null, CacheRoot = @"C:\MyApp\cache" };
            services.AddSingleton<IMediaPlayer>(new FakePlayer());
        });

        using var app = builder.Build();

        Assert.Equal(@"C:\MyApp\cache", app.Services.GetRequiredService<MediaPlayerOptions>().Access.CacheRoot);
    }
