...
public static class FileSystemExtensions
{
        ...
        Action<FileUpdateQueueOptions, IServiceCollection> configure)
    {
        ...
        builder.Services.TryAddSingleton<IFileUpdateQueue>(provider =>
        {
            ...
            // `UseWindows`/`UseAndroid`/`UseIOS` may run after this. This is the file system's version of what
            // `IMediaCapability` and `IMediaStreamConversion` are for media: the ENGINE is portable, the
