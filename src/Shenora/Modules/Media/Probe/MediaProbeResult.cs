...
/// </param>
/// <param name="Width">
/// Video width in pixels, when known. <b>The video peer of <paramref name="SampleRate"/>, and it is needed
/// for the same reason</b>: a platform video codec is configured with its dimensions before it is fed
/// anything. ⚠ A decoder configured at the wrong size does not fail — it produces a picture that is
/// stretched, cropped or green.
/// </param>
/// <param name="Height">Video height in pixels, when known. See <paramref name="Width"/>.</param>
/// <param name="FrameRate">
/// Frames per second, when known. Only an encoder HINT — the sample timing that actually lands in the file
/// comes from each frame's presentation time, never from this. A container commonly reports a nominal rate
/// for a variable-rate stream, so treating it as truth is how a file ends up playing at the wrong speed.
/// </param>
