...
public sealed class OperationRegistry : IOperationRegistry, IDisposable
{
    ...
    /// <see cref="Shenora.Core.Events.IEventBus"/>, not strict equality: no requested scope = every scope,
    /// AND an unscoped (global) entry matches any requested scope too — both event buses
    /// (<c>Shenora.Core.Events.EventBus</c>, the TS <c>ShenoraEventBus</c>) already apply this, so an entry
    ...
    /// (<see cref="Shenora.Core.Events.IEventBus"/>).
    /// </summary>
