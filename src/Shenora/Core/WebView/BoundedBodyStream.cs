...
internal sealed class BoundedBodyStream : Stream
{
    ...
    public BoundedBodyStream(Stream inner, long length, ILogger? log = null)
    {
        ...
        // takes the read the close hangs off — and an empty file/range would leak its handle for as long
        // as the platform holds this response, on iOS indefinitely.
    ...
    /// call of its own, so an ordinary nonzero short read passes straight through as legal
    /// <see cref="Stream.Read(byte[], int, int)"/> behaviour. Only a <c>0</c> returned while the bound
    /// still owes bytes means the source is exhausted, and that throws
    ...
    /// Only Android turns it into a page-visible failed load; the per-shell arms are in
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
        ...
        catch (Exception ex)
        {
            ...
            // already out. Logged HERE rather than in Dispose, which iOS never calls.
            // ⚠ RETHROWN UNCHANGED — Android's shell translates it into a Java.IO.IOException Chromium
            // catches; swallowing it here turns that good outcome into the silent one.
    ...
    /// <summary>
    /// Always throws. No platform has been observed to seek a body reporting <see cref="CanSeek"/>
    /// <c>== false</c> — measured in <c>docs/design/mobile-shells.md</c>.
    /// </summary>
    ...
    /// case is a second call on the same body and a two-thread race must also be safe.
    /// </summary>
