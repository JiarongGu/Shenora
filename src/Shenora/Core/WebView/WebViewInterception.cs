...
/// in <c>Shenora</c> beside the request and response types it speaks (D19/D20).
/// </para>
/// <para>
...
public interface IWebViewInterceptor
{
    ...
    /// <para>
    /// 🔴 <b>On the MOBILE shells the cost of blocking is not a pause, it is a DEADLOCK, and this is a
    /// lesson the kit paid for rather than a caution.</b> Both mobile platforms need the status line and
    /// headers by the time the platform event returns, so the shell resolves this pipeline SYNCHRONOUSLY
    /// (<c>GetAwaiter().GetResult()</c>) on the main thread. A middleware that <c>await</c>s anything whose
    /// continuation is posted back to that thread — any <c>await</c> without
    /// <c>ConfigureAwait(false)</c> — therefore waits on a thread only it could free. ⚠ The symptom names
    /// nothing: on iOS the app stays alive, the request is the last line it ever logs, and every later
    /// native evaluation simply never answers. The kit's own fragment repair did exactly this and it
    /// blocked a real adopter's bug for three days (see <c>MobileWebViewInterceptor</c>).
    /// </para>
    /// <para>
    /// So: do the slow work BEFORE the request (warm a cache at startup), or hand back a lazy body and do
    /// it as the platform reads — and if you must await inside the middleware itself,
    /// <c>ConfigureAwait(false)</c> is not a style choice here.
    /// </para>
    /// </param>
