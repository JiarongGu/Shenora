...
/// <para>
/// 🔴 <b>Events answer "what happened"; the hooks on <see cref="SessionBrowserOptions"/> answer "what
/// should happen".</b> An event has many subscribers and no return value; a hook has exactly one owner
/// and a decision the browser obeys. Conflating them is why this surface was lopsided — every policy
/// decision was the kit's and nothing was observable.
/// </para>
/// <para>
/// ⚠ <b>These go on the EXISTING bus rather than on N bespoke <c>OnX</c> taps.</b> The kit already has
/// pub/sub with scope matching, wildcards and guarded handlers; a second subscription idiom beside it
/// would be drift, not a feature. Subscribe with
...
/// indistinguishable — a subscriber watching for a login would react to another session's redirect.
/// Every session type the kit ships supplies its own id; an unscoped emit is a global broadcast and
/// reaches every subscriber, which is only correct when there is exactly one session.
/// </para>
/// </summary>
public static class SessionEvents
{
    ...
    /// <para>
    /// ⚠ <b>OFF unless <see cref="SessionBrowserOptions.ObserveResponse"/> selects it</b>, because this
    /// is the only event here that fires per SUBRESOURCE: a single page load is hundreds of images,
    /// stylesheets and XHRs, and building a header list for each one is a cost nothing should pay by
    /// accident. The predicate is both the on-switch and the cost control.
    /// </para>
    /// </summary>
    ...
    /// <see cref="SessionNavigationResult"/>. This is the one that makes a REDIRECT-driven load
    /// visible: today it can only be observed by awaiting your own <c>NavigateAsync</c>, so a page
    /// that bounced somewhere else on its own arrived unannounced.
    /// </summary>
    ...
    /// <summary>
    /// The address changed WITHOUT a navigation — payload <see cref="SessionSource"/>. This is the SPA
    /// signal: a <c>history.pushState</c> route change fires no navigation at all, so an app watching
    /// only <see cref="NavigationCompleted"/> never learns the user moved.
    /// </summary>
...
/// GOING while <paramref name="Title"/> is still the outgoing document's — the new page has no title
/// yet. Stated because "url and title" reads like one coherent page and for that one event it is two.
/// </para>
/// </summary>
...
/// response carries <c>Set-Cookie</c> as it happens, along with the redirect that usually accompanies
/// it — one mechanism serving cookie capture, redirect tracing and API observation, none of them named
/// for a scenario.
/// </para>
/// <para>
/// ⚠ <b>What it does NOT see: a cookie set by JS</b> (<c>document.cookie</c>). Nearly every auth cookie
/// is server-set, so the real case is covered — but the limit is documented rather than discovered.
/// Read the actual jar with <see cref="SessionController.GetCookiesAsync"/>.
/// </para>
/// </summary>
...
/// <param name="Headers">
/// The response headers as reported, in order. ⚠ <b>A LIST, not a dictionary, and deliberately</b>:
/// <c>Set-Cookie</c> legitimately repeats, and a map keyed by name would keep one and silently drop the
/// rest — losing exactly the header this event exists to carry. (That the SDK's own enumerator yields a
/// repeated name as separate entries is INFERRED from its shape, not measured — it exposes
/// <c>GetHeaders(name)</c> returning many values for one name. A list cannot be worse than a map here
/// either way, which is why the choice does not wait on a live browser to settle it.)
/// </param>
...
/// content that was already consumed or is still streaming yields empty rather than failing the event.
/// </param>
...
/// <param name="Terminal">
/// Whether this kind means the SESSION is dead. False for the routine ones — a GPU-driver reset raises
/// a failure on a perfectly live page — so a subscriber can log everything and act on the two that
/// matter. See <c>SessionBrowser.IsTerminal</c>.
/// </param>
