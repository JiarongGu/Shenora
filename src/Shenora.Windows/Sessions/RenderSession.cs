...
/// <summary>
/// A leased, driveable off-screen browser session: the caller navigates it, runs its OWN JS on the live
/// page, reads the HTML and calls CDP methods. The pool owns the WebView2 and the UI thread; the caller
/// owns ALL interpretation (its settle poll, its DOM analysis). Every WebView2 touch marshals onto the
/// pool's UI-thread anchor. <see cref="DisposeAsync"/> returns the instance to the pool — idempotent and
/// guarded: after dispose, or if the message loop is gone, every op fails gracefully.
///
...
public sealed class RenderSession : IAsyncDisposable
{
    ...
    /// ⚠ <b>It belongs to the LEASE, not to the pooled browser</b> — the same browser gets a different
    /// id next time it is leased, so a subscription outliving this session stops receiving anything
    /// rather than picking up the next tenant's pages.
    /// </summary>
    public string Id { get; }

    /// <summary>
    /// Navigate to an absolute http(s) URL and wait for the DOCUMENT to load only
    /// (NavigationCompleted) — NOT for JS to settle; the caller decides "settled" itself via script
    /// polling. <see cref="RenderSessionPoolOptions.NavigationTimeout"/> caps the wait so a hung load
    /// can't wedge the leased instance, and every navigation must first pass
    ...
    public Task NavigateAsync(string url, CancellationToken cancellationToken = default) => OnUiAsync(async () =>
    {
        ...
        // unvetted CROSS-ORIGIN hop from here on, so a 302 to somewhere the guard never saw can't be
        // followed.
        //
        ...
        try
        {
            ...
            // WhenAny never throws, and the cap firing and the CALLER cancelling both complete the
            // Delay task — but they mean different things. The cap is a soft "return what's there";
            // the caller's own token means "I gave up", which MUST surface so it can't be mistaken
            // for a completed load.
    ...
    /// instance back, and the pool resets it to about:blank and releases the capacity slot. Never throws
    ...
    /// <summary>
    /// Run <paramref name="work"/> ON THE UI THREAD and await its result — the one marshal every op
    /// uses. A disposed session, a dead message loop or a thrown delegate all surface as the delegate's
    /// own exception path, never a wedge. Bounded by
    /// <see cref="RenderSessionPoolOptions.OpTimeout"/>, and an operation the UI thread never finishes
    /// POISONS the instance (see <see cref="RunBoundedAsync{T}"/>).
    /// </summary>
    private Task<T> OnUiAsync<T>(Func<Task<T>> work, CancellationToken cancellationToken)
    {
        ...
        // The ONE marshal owner. It observes the token via WaitAsync, so the CALLER always escapes even
        // when the UI thread never runs the body — an op against a page whose JS thread is blocked
        // otherwise never cancels, the lease never returns, and the pool's permit is gone for good.
        return RunBoundedAsync(work, cancellationToken);
    }

    /// <summary>
    /// The other half: escaping the await hands the CALLER back but leaves the wedged page in the pool,
    /// so this adds a BOUNDED wait (<see cref="RenderSessionPoolOptions.OpTimeout"/>, because a caller
    /// that passes no token has no escape at all) and POISONS the instance when the body never
    /// completed, so <see cref="RenderSessionPool.Return"/> discards it instead of re-pooling it.
    /// <para>
    /// ⚠ "Never completed" is TRACKED rather than inferred: a body that ran and threw (a bad URL, a
    /// guard refusal, a caller token observed INSIDE the body) leaves the instance perfectly reusable,
    /// and discarding it would cost seconds of browser startup on every ordinary error.
    /// </para>
    /// </summary>
    private async Task<T> RunBoundedAsync<T>(Func<Task<T>> work, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (Volatile.Read(ref finished) == 0 && bounded.IsCancellationRequested)
            {
                ...
                // The original stays as the inner exception.
