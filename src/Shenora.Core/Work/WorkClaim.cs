...
/// <summary>How a <see cref="WorkClaim"/> excludes other work on a conflicting key.</summary>
...
/// <summary>
/// One resource a <see cref="WorkRequest"/> needs before it may run: a key inside a named
...
public readonly record struct WorkClaim(string Scope, string Key, ClaimMode Mode)
{
    ...
    public static WorkClaim Exclusive(string scope, string key) => new(scope, key, ClaimMode.Exclusive);

    /// <summary>A shared claim on <paramref name="key"/> within <paramref name="scope"/>.</summary>
    public static WorkClaim Shared(string scope, string key) => new(scope, key, ClaimMode.Shared);
}
