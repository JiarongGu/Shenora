...
| `2026-08-03-shenora-media-design.md` | Media + playback: §0's harvest from THREE siblings (two of which credit each other in their own XML), the mobile platform capabilities verified by compiling, measured engine sizes, and §0f's finding that a transcode layer is `IMissionScheduler` + `PathClaims` + `Files.BeginReplace` + one new decision function | Design; retire once built (WHYs → `DECISIONS.md`) |
