...
- 🔴 **`IMediaPlayer` — the host plays, the page drives (D54).** A portable contract in `Shenora.Core`
  ...
  - 🔴 **`MediaPlayer` + `MediaPlayerEvents` — the interceptor's media route is now the PLAYER's output
    pipe, not a parallel feature (D58).** `MediaPlayer` owns probe → plan → resolve-the-URL, publishes
    load/play/pause/seek/rate/unload to the page over `IEventBus`, and takes the page's element state back
    through `MediaPlayer.Report`. So a media request reaching the interceptor is a question **.NET**
    answers, and the page never decides anything about format — it renders what it is handed.
