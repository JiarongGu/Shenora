---
name: skill-loader
description: Check whether a Shenora skill already covers the task (loading the right docs/knowledge, finding a code pattern or extraction source, scaffolding an IPC module or a native service, or running an end-of-phase review) so you use the skill instead of hand-rolling. Use at the start of a task to pick the right tool.
---
...
1. Match the task to a row. A non-trivial start usually pairs `/doc-loader` + `/pattern-finder`;
   ...
   `/phase-review`. After a fix lands there is no log to append — the commit message carries the root
   cause and a reusable invariant becomes a knowledge rule (`dev.mjs knowledge new <name>`).
